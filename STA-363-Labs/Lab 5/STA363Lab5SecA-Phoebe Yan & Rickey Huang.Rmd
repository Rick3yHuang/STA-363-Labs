...
See from the somputation result for ajusted $R^2$ s above, the $R_{adj}^{2}$ of the model fitted with one X is $0.2202324$. The $R_{adj}$ for the model with two Xs is 0.2246977.
...
From the plote made from the previous question, we can see that the model with health Good, health Fair, gender female and the intercept has an $R_{adj}^{2}$ of 0.23
...
In the lowest $R_{adj}^{2}$ ($0.64$) model, the feature used are the desired weight in pounds and the intercept. The model with the highest $R_{adj}^{2}$ ($0.67$) uses features of health Very Good, health Good, health Fair, exerany, health plan, height, desired weight in pounds, age, gender female and the intercept.
