...
```{r}
# Use LOOCV technique to do the cross validation
# Store the number of rows into n
n <- nrow(college)
# Create space to store the residual sum of squares for both models
rss_Tree2 <- rep(0,n)
rss_LSLR2 <- rep(0,n)
# For loop to perform the LOOCV technique
for (i in 1:n) {
  # Train the models with each i
  Tree2CV <- rpart(Grad.Rate ~ S.F.Ratio, method = "anova", data = college[-i,], cp = 0.02, maxdepth = 1)
  LSLR2CV <- lm(Grad.Rate ~ S.F.Ratio, data = college[-i,])
  # Make prediction with both models for each i
  yhat_Tree2 <- predict(Tree2CV, newdata = college[i,])
  yhat_LSLR2 <- predict(LSLR2CV, newdata = college[i,])
  # Calculate the residual sum of squares for each model
  rss_Tree2[i] <- (college$Grad.Rate - yhat_Tree2)^2
  rss_LSLR2[i] <- (college$Grad.Rate - yhat_LSLR2)^2
}
# Compute the test MSE for both the models
MSE_Tree2 <- mean(rss_Tree2)
MSE_LSLR2 <- mean(rss_LSLR2)
```
